...
async def patch_note_content(db: AsyncSession, note_id: str, operations: list[dict]) -> Note:
    """Apply string-match patch operations to a note's content.

    Each operation has old_string and new_string.
    - old_string must be non-empty and appear exactly once in the current content.
    - new_string replaces old_string (empty new_string = deletion).
    - Operations are applied sequentially — each sees the result of the previous.
    """
    note = await get_note(db, note_id)
    if note is None:
    ...
    for i, op in enumerate(operations):
        ...
        if not old:
            raise ValueError(
                f"Operation {i}: old_string cannot be empty. "
                "To insert text, match a nearby line and include it in new_string."
            )
        ...
        if count == 0:
            ...
            raise ValueError(
                f"Operation {i}: old_string not found in note content. "
                f"Read the note again — the content may have changed.\n"
                f"Searched for: {preview!r}"
            )
        if count > 1:
            raise ValueError(
                f"Operation {i}: old_string appears {count} times. "
                "Include more surrounding context in old_string to make it unique."
            )

        content = content.replace(old, new, 1)

    return await update_note(db, note_id, content=content)
